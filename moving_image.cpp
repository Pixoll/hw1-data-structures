...
image_action::image_action(IMAGE_ACTION_TYPE type, int argument, IMAGE_ACTION_TYPE origin) {
    this->type     = type;
    this->origin   = origin;
    this->argument = argument;
}

image_action::image_action(IMAGE_ACTION_TYPE type, int argument) : image_action(type, argument, type) {}

image_action::~image_action() {}

IMAGE_ACTION_TYPE image_action::get_type() {
    return this->type;
}

int image_action::get_argument() {
    return this->argument;
}

string image_action::to_string() {
...
image_action *image_action::get_inverse() {
...
image_action *image_action::copy_with_origin(IMAGE_ACTION_TYPE origin) {
...
moving_image::exception::exception(const char *msg) : message(msg) {}

moving_image::exception::~exception() {}

const char *moving_image::exception::what() const throw() {
    return this->message;
}

void moving_image::draw(const char *file_name) {
...
void moving_image::move_left(int pixels) {
    pixels %= H_IMG;
    if (pixels < 0)
        pixels += H_IMG;
...
void moving_image::move_right(int pixels) {
    pixels %= H_IMG;
    if (pixels < 0)
        pixels += H_IMG;
...
void moving_image::move_up(int pixels) {
    pixels %= W_IMG;
    if (pixels < 0)
        pixels += W_IMG;
...
void moving_image::move_down(int pixels) {
    pixels %= W_IMG;
    if (pixels < 0)
        pixels += W_IMG;
...
void moving_image::rotate() {
    this->_save_action(ROTATE_COUNTERCLOCKWISE);
}

void moving_image::undo() {
...
void moving_image::_save_action(IMAGE_ACTION_TYPE type, int argument) {
    if (type == UNDO) {
...
void moving_image::_apply_undone(stack<image_action *> inverses) {
    while (!this->undone.empty()) {
        image_action *action = this->undone.top();
        this->history.push_back(action);
        inverses.push(action);
...
void moving_image::_draw(const char *file_name) {
...
void moving_image::_draw(const string file_name) {
    this->_draw(file_name.c_str());
}

void moving_image::_print_history() {
...
void moving_image::_move(int pixels, bool horizontal) {
    if (pixels == 0)
        return;
...
void moving_image::_swap_pixels(int i1, int j1, int i2, int j2) {
    const rgb temp      = this->image[i1][j1];
    this->image[i1][j1] = this->image[i2][j2];
    this->image[i2][j2] = temp;
}

void moving_image::_block_swap(int fi, int si, int length, bool horizontal, int pivot) {
    if (horizontal) {
...
void moving_image::_rotate(bool clockwise) {
    if (clockwise) {
...
void moving_image::_perform_action(image_action *action, bool inverse) {
...
void moving_image::_reset_image() {
